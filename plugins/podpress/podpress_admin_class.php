...
*/
	class podPressAdmin_class extends podPress_class
	{
		...
		function post_form($entryType = 'post') {
			...
			$newMediaDefaults['dimensionW'] = '300'; //BB Dev
			echo "newMediaDefaults['dimensionW'] = '".$newMediaDefaults['dimensionW']."';\n";
			$newMediaDefaults['dimensionH'] = '30'; //BB Dev
			...
			// BB Dev: intel <-
			$newMediaDefaults['alreadyOpen'] = 'false';
			echo "newMediaDefaults['alreadyOpen'] = ".$newMediaDefaults['alreadyOpen'].";\n";
			// BB Dev: intel ->
...
// BB Dev: intel <-
				if(!isset($post->podPressMedia[$num]['alreadyOpen']) || $post->podPressMedia[$num]['alreadyOpen'] == false || $post->podPressMedia[$num]['alreadyOpen'] == 'false') {
					$post->podPressMedia[$num]['alreadyOpen'] = 'false';
				} else {
					$post->podPressMedia[$num]['alreadyOpen'] = 'true';
				}
// BB Dev: intel ->
			...
			echo '<div id="podPressstuff" class="meta-box-sortables ui-sortable">'."\n"; # BB Develop: add postbox admin syles
			...
			while ($num < $this->settings['maxMediaFiles']) {
				...
				echo '				<table border="0" style="width: 100%;"><tr><td style="border-right-style: none; text-align: left;"><label><strong>'.__('Media File', 'podpress').' #'.$display_num.':</strong> [media '.$display_num.'] </label></td>'; //BB Dev: pg
...
BB Dev */
				...
				do_action('podPress_customMediaData', array($actionMedia));
				echo '					<tr>'."\n";
				echo '						<td style="vertical-align: middle;" nowrap="nowrap">'."\n";
				echo '							'.__('Tag (ID3) Info', 'podpress').":\n";
				echo '						</td>'."\n";
				echo '						<td>'."\n";
				echo '							<input type="button" name="podPressMedia['.$num.'][mp3_details_button]" id="podPressMedia_'.$num.'_mp3_detailsbutton_" value="Show" onclick="javascript: podPressShowHideMP3Details('.$num.');"/>'."\n";
		...
		function edit_category_form($input) {
			...
			echo '					<br/>';
			echo '					<input name="iTunesFeedID" id="iTunesFeedID" type="text" value="'.htmlentities($data['iTunesFeedID'], ENT_QUOTES, $blog_charset).'" size="10" />';
			...
			echo '					<br/>';
			echo '					<input type="text" id="podcastFeedURL" name="podcastFeedURL" size="40" value="'.htmlentities(stripslashes($data['podcastFeedURL']), ENT_QUOTES, $blog_charset).'" /><br />'.__('The RSS Feed URL to your podcast.', 'podpress');
			echo '				</td>'."\n";
			echo '			</tr>'."\n";
			...
			echo '					</select>'."\n";
			echo '					<input type="hidden" id="global_blogname" value="'.htmlentities(stripslashes(get_option('blogname')), ENT_QUOTES, $blog_charset).'" /></td>'."\n";
			echo '				</td>'."\n";
			echo '			</tr>'."\n";
			...
			echo '					</select>'."\n";
			echo '					<input type="hidden" id="global_iTunesSummary" value="'.htmlentities(stripslashes($this->settings['iTunes']['summary']), ENT_QUOTES, $blog_charset).'" />'."\n";
			echo '					<div id="iTunesSummaryWrapper" style="display: none;">'."\n";
			echo '						<textarea name="iTunesSummary" id="iTunesSummary" rows="4" cols="40" onchange="javascript: podPress_updateCategoryCasting();">'.htmlentities(stripslashes($data['iTunesSummary']), ENT_QUOTES, $blog_charset).'</textarea>'."\n";
			echo '					</div>'."\n";
			...
			echo '					</select>'."\n";
			echo '					<input type="hidden" id="global_blogdescription" value="'.htmlentities(stripslashes(get_option('blogdescription')), ENT_QUOTES, $blog_charset).'" />'."\n";
			echo '				</td>'."\n";
			echo '			</tr>'."\n";
			...
			echo '						<input type="text" name="iTunesAuthor" size="40" id="iTunesAuthor" value="'.htmlentities(stripslashes($data['iTunesAuthor']), ENT_QUOTES, $blog_charset).'" onchange="javascript: podPress_updateCategoryCasting();"/>';
			echo '						<input type="hidden" id="global_iTunesAuthor" value="'.htmlentities(stripslashes($this->settings['iTunes']['author']), ENT_QUOTES, $blog_charset).'" />'."\n";
			echo '					</div>'."\n";
			...
			echo '						<input type="text" name="iTunesAuthorEmail" size="40" id="iTunesAuthorEmail" value="'.htmlentities(stripslashes($data['iTunesAuthorEmail']), ENT_QUOTES, $blog_charset).'" onchange="javascript: podPress_updateCategoryCasting();"/>';
			echo '						<input type="hidden" id="global_iTunesAuthorEmail" value="'.htmlentities(stripslashes(get_option('admin_email')), ENT_QUOTES, $blog_charset).'" />'."\n";
			echo '					</div>'."\n";
			...
			echo '						<textarea name="iTunesSubtitle" rows="4" cols="40">'.htmlentities(stripslashes($data['iTunesSubtitle']), ENT_QUOTES, $blog_charset).'</textarea>'."\n";
			echo '					</div>'."\n";
			...
			echo '						'.__('Separate multiples with commas', 'podpress').', '.__('max 8', 'podpress').'<br/><textarea name="iTunesKeywords" rows="4" cols="40">'.htmlentities(stripslashes($data['iTunesKeywords']), ENT_QUOTES, $blog_charset).'</textarea>'."\n";
			echo '					</div>'."\n";
			...
			echo '					<label for="rss_copyrightChoice"><strong>'.__('Feed Copyright / license name', 'podpress').'</strong></label> <a href="javascript:void(null);" onclick="javascript: podPressShowHideRow(\'rss_copyrightHelp\');">(?)</a>:'."\n";
			echo '					<br/>';
			...
			echo '					<label for="rss_license_urlChoice"><strong>'.__('URL to the full Copyright / license text', 'podpress').'</strong></label> <a href="javascript:void(null);" onclick="javascript: podPressShowHideRow(\'rss_license_urlHelp\');">(?)</a>:'."\n";
			echo '					<br/>';
			echo '					<select id="rss_license_urlChoice" name="rss_license_urlChoice" onchange="javascript: podPress_updateCategoryCasting();">'."\n";
		...
		function edit_category($cat_ID) {
			...
			$data['rss_license_url'] = $_POST['rss_license_url'];
