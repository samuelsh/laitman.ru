...
// BB Dev: pg <-
		var $old_podcastTag          = '[display_podcast]';
		var $podcastTag          = '[media]';
		var $podcastTagEx	 = '[media %d]'; //must be used within fprint, e.g. fprint($podcastTagEx, 2)
		var $podcastTagEx_regexp = '/\[media\s*%s\]/'; //must be used within preg_replace, e.g. preg_replace(sprintf(podcastTagEx_regexp,2), 'replace', $s);
// BB Dev: pg ->
		...
		function podPress_class() {
			...
			$this->tempFileURLPath = get_option('siteurl').'/'.get_option('upload_path').'/podpress_temp';
		...
		function checkSettings() {
			...
			if(empty($this->settings['mediaWebPath'])) {
				$this->settings['mediaWebPath'] = get_option('siteurl').'/wp-content/uploads';
			}
			...
			if($method != false) {
				if($this->settings['enableStats']) {
					...
					if($this->settings['statMethod'] == 'download.mp3') {
						...
						$url = get_option('siteurl').'/podpress_trac/'.$method.'/'.$postID.'/'.$mediaNum.'/'.$filename_part;
					}
		...
		function checkWritableTempFileDir($showErrors = true) {
			...
			if (file_exists($this->tempFileSystemPath)) {
				if(is_writable($this->tempFileSystemPath)) {
					...
					if($showErrors) {
						echo '<p>'.__('Your uploads/podpress_temp directory is not writable. Please set permissions as needed, and make sure <a href="'.get_option('siteurl').'/wp-admin/options-misc.php">configuration</a> is correct.', 'podpress').'<br />Currently set to:<code>'.get_option('upload_path')."/podpress_temp</code></p>\n";
					}
					return false;
				}
			} elseif (!file_exists($this->uploadPath)) {
				if($showErrors) {
					echo '<p>'.__('Your WordPress upload directory does not exist. Please create it and make sure <a href="'.get_option('siteurl').'/wp-admin/options-misc.php">configuration</a> is correct.', 'podpress').'<br />Currently set to:<code>'.get_option('upload_path')."</code></p>\n";
				}
				return false;
			} elseif (!is_writable($this->uploadPath)) {
				if($showErrors) {
					echo '<p>'.__('Your WordPress upload directory is not writable. Please set permissions as needed, and make sure <a href="'.get_option('siteurl').'/wp-admin/options-misc.php">configuration</a> is correct.', 'podpress').'<br />Currently set to:<code>'.get_option('upload_path')."</code></p>\n";
				}
				return false;
			} else {
		...
		function insert_content($content = '') {
			...
			if(isset($this->tempContentAddedTo[$post->ID])) {
				if(is_feed()) {
					return preg_replace(sprintf($this->podcastTagEx_regexp,'\d*'), '', $content); //BB Dev: pg
				}
				return $content;
			} else {
				$this->tempContentAddedTo[$post->ID] = true;
			...
			$content = str_replace($this->old_podcastTag,$this->podcastTag,$content); //BB Dev: pg: replace [display_podcast] -> [media]
			...
			//BB Dev: $hasLocationDefined = (bool)strstr($content, $this->podcastTag);
			$hasLocationDefined = (bool)preg_match(sprintf($this->podcastTagEx_regexp,'\d*'), $content); // BB Dev: pg
...
// BB Dev: intel ->
				...
				if(isset($post->podPressMedia[$key]['authorized'])) {
				...
				$post->podPressMedia[$key]['URI'] = isset($post->podPressMedia[$key]['URI_orig']) ? $post->podPressMedia[$key]['URI_orig'] : null;
				unset($post->podPressMedia[$key]['URI_orig']);
			}

			if(is_feed()) {
				//BB Dev return str_replace($this->podcastTag,'<br/>'.$podPressRSSContent,$content);
				return preg_replace(sprintf($this->podcastTagEx_regexp,'\d*'), '<br/>'.$podPressRSSContent, $content); //BB Dev: pg
			}
			...
			if(!$this->settings['compatibilityChecks']['wp_head']) {
				...
				$podPressContent = podPress_webContent($podPressTemplateData, $content, $this->podcastTagEx_regexp); //BB Dev: pg
			}
			//BB Dev return str_replace($this->podcastTag,$podPressContent,$content);
			return $podPressContent; //BB Dev: pg
		}
