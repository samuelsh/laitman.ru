...
*/
	if(!function_exists('podPress_webContent')) {
		...
		foreach ($podPressTemplateData['files'] as $key=>$val) {
			...
			if(!$val['authorized']) {
				$podPressContent .= ' <a href="'.get_option('siteurl').'/wp-login.php">(Protected Content)</a><br/>'."\n";
			} else {
				if($val['enablePlayer']) {
					if($dividerNeeded) {
					...
					if ($val['type'] == 'embed_youtube')
					{
						...
						$uri = "https://" . $match[1]; //BB Dev: grisha
					} else {
						$uri = $val['URI'];
					}
					// BB-Dev (som): podpress - wpematico conflict solved
					...
					$podPressContent .= "<a href=\"".$uri."\" rel=\"nofollow\"><span id=\"podPressPlayerSpace_".$GLOBALS['podPressPlayer']."_PlayLink\">".__('Play Now', 'podpress')."</span></a>";
					$dividerNeeded = true;
...
//BB Dev > start
					if (preg_match('%youtube|laitman\.ru%', $val['URI'], $match) == 0 && preg_match('%^.+://([^/]+)/(.+)$%', $val['URI'], $match) > 0) { //BB Dev: Added regex 'laitman\.ru' to preven adding '/download/' to URL
						// $uri = "https://" . $match[1] . '/download/' . $match[2]; //ZZZ
						$uri = "https://" . $match[1] . '/' . $match[2]; //ZZZ
					} else {
						$uri = $val['URI'];
					}
...
// BB Dev: grisha > end
				...
				$podPressContent .= '<span class="podPressPlayerSpace" id="podPressPlayerSpace_'.$GLOBALS['podPressPlayer'].'" style="display: block;z-index: 1;">&nbsp;</span>'."\n";
			}
//BB Dev: pg <-
