...
*/
	...
	function podPress_rss2_head() {
		...
		if($podPress->settings['category_data']['categoryCasting'] == 'true') {
			...
			if (0 >= strlen(trim($data['author']))) {
				if (isset($podPress->settings['iTunesAuthor']) && 0 < strlen($podPress->settings['iTunesAuthor'])) {
	...
	function podPress_xspf_playlist() {
		...
		header('Content-type: application/xspf+xml; charset=' . get_option('blog_charset'), true);
	...
	function podPress_getEnclosureTags($feedtype = 'rss2') {
		...
		if(is_array($post->podPressMedia)) {
			...
			while (list($key, $val) = each($post->podPressMedia)) {
				...
				if ((isset($post->podPressMedia[$key]['rss']) && $post->podPressMedia[$key]['rss'] == 'on') || $post->podPressMedia[$key]['atom'] == 'on' || $preferredFormat == true) {
					if ($feedtype == 'atom' && $post->podPressMedia[$key]['atom'] == 'on') {
						...
						if((isset($post->podPressMedia[$key]['rss']) and $post->podPressMedia[$key]['rss'] == 'on') AND FALSE === $same_enclosure_URL_in_postmeta_exists ) {
