...
*/
	...
	// BB develop: cut the string by first or last space
	function cut_by_space($str, $from_end = true)
	{
		$str_arr = explode(" ", $str);
		if ($from_end)
			array_pop($str_arr);
	...
	// BB develop: the resulting string should be UTF-8 compliant.
	// The easiest way to ensure this is to cut it up to the latest
	// space symbol (which is surely UTF-8 compliant). If such symbol
	// doesn't exist, return te empty string.		
	function podPress_stringLimiter($str, $len, $snipMiddle = false)
	{
		if (strlen($str) > $len) {
			if($snipMiddle) {
				...
				$start = substr($str, 0, $startlen);
				$end = substr($str, $endlen);

				// cut $start up to the latest space
				$start = cut_by_space($start);
				// cut $end from the first space
				$end = cut_by_space($end, false);
				...
				// cut $res up to the latest space
				$res = cut_by_space($res);
	...
	function podPress_feedButtons_control() {
		...
		<?php
		if($podPress->settings['podcastFeedURL'] != get_option('rss2_url')) {
		?>
