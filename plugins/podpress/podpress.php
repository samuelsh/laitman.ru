...
if (! function_exists ( 'memory_get_usage' )) {
	...
	if (! function_exists ( 'podPress_bytes' )) {
		function podPress_bytes($i) {
			return $i;
		}
		function podPress_checkmem() {
			return;
		}
	}
	...
	function podPress_checkmem($txt, $start = false) {
		...
		if (isset ( $_GET ['podpress_showmem'] )) {
			...
			if ($start) {
				$podPress_memoryUsage [$txt] = array (
						'start' => $mem 
				);
			} else {
...
if (! class_exists ( 'podPress_class' )) {
	...
	function podPress_wp_head() {
		...
		if (! $podPress_inAdmin) {
			if (! isset ( $podPress->settings ['compatibilityChecks'] ['themeTested'] )) {
				$podPress->settings ['compatibilityChecks'] ['themeTested'] = true;
				podPress_update_option ( 'podPress_config', $podPress->settings );
			}
			if (! isset ( $podPress->settings ['compatibilityChecks'] ['wp_head'] )) {
		...
		if (isset ( $podPress->settings ['enablePodangoIntegration'] ) || (isset ( $podPress->settings ['mp3Player'] ) && $podPress->settings ['mp3Player'] != '1pixelout')) {
	...
	function podPress_addFeedHooks() {
		...
		if (! $podPress_feedHooksAdded) {
			...
			add_action ( 'the_content_rss', array (
					&$podPress,
					'insert_content' 
			) );
...
add_action ( 'xmlrpc-mw_newPost', array (
		&$podPress,
		'xmlrpc_post_addMedia' 
) );
add_action ( 'xmlrpc-mw_editPost', array (
		&$podPress,
		'xmlrpc_post_addMedia' 
) );
...
add_action ( 'get_the_excerpt', array (
		&$podPress,
		'insert_the_excerpt' 
), 1 );
add_action ( 'the_excerpt', array (
		&$podPress,
...
if (isset ( $podPress_inAdmin )) {
	add_action ( 'activate_podpress/podpress.php', array (
			&$podPress,
			'activate' 
	) );
	add_action ( 'deactivate_podpress/podpress.php', array (
			&$podPress,
			'deactivate' 
	) );
	...
	if (TRUE == version_compare ( $wp_version, '2.5', '>=' )) {
		...
		add_action ( 'simple_edit_form', array (
				&$podPress,
				'post_form' 
		) );
	...
	add_action ( 'edit_category_form', array (
			&$podPress,
			'edit_category_form' 
	) );
	add_action ( 'edit_category', array (
			&$podPress,
			'edit_category' 
	) );
	...
	/* stuff for editing settings */
	$wp_importers ['podcast'] = array (
			'Podcast RSS2',
			'podPress import of posts from a Podcast RSS2 feed.',
			array (
					&$podPress,
					'import_dispatch' 
			) 
