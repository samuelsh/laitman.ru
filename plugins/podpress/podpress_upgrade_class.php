...
*/
	class podPressUpgrade_class extends podPress_class
	{
		function podPressUpgrade_class($current = 0) {
			GLOBAL $wpdb, $podPress;
			if(!$current) {
				$current = 0;
			}
			if(PODPRESS_VERSION <= $current) {
				$this->podPress_class();
				return;
			}
			...
			$this->activate();
			while(PODPRESS_VERSION > $current) {
				...
				switch($current) {
					...
					case '3.8':
						...
						if($this->settings['audioWebPath'] == '') {
							add_option('podPress_mediaWebPath', $this->settings['audioWebPath'], "", true);
							$this->settings['audioWebPath'] = $this->settings['audioWebPath'];
							delete_option('podPress_audioWebPath'); 
						}

						if($this->settings['audioFilePath'] == '') {
							add_option('podPress_mediaFilePath', $this->settings['audioFilePath'], "", true);
							...
							foreach ($posts as $post) {
								$sql = "SELECT meta_key, meta_value
								        FROM ".$wpdb->prefix."postmeta 
												WHERE meta_key  IN('podPress_podcastStandardAudio', 
					...
					case '5.6':
						...
						if($y) {
							...
							add_option('podPress_config', $newSettings, "", true);
						}
						break;
					case '5.7':
					...
					case '6.7':
						...
						if ($posts) {
							...
							foreach($mappings as $key => $val) {
								//echo ('UPDATE '.$wpdb->prefix."podpress_statcounts SET postID = '".$val."' WHERE media='".$key."'");
								//echo ('UPDATE '.$wpdb->prefix."podpress_stats SET postID = '".$val."' WHERE media='".$key."'");
