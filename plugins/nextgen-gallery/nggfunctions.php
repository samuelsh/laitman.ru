...
function nggCreateImageBrowser($picarray, $template = '') {
    ...
    // BB Develop
	$args['show'] = "gallery";
	$slideshow_links = explode('\?', $nggRewrite->get_permalink($args));
	$picture->slideshow_link = $slideshow_links[0] . '?show=gallery';
	$ngg_options = nggGallery::get_option('ngg_options');
	$picture->slideshow_link_text = nggGallery::i18n($ngg_options['galTextGallery']);
