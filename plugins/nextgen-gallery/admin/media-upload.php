...
// BB developer: the NextGen is the most used tab, so we'd like
// it to be a first one and a selected one
add_filter('media_upload_default_tab', 'ngg_wp_default_tab');
function ngg_wp_default_tab() {
	return "nextgen";
}
...
function media_upload_nextgen_form($errors) {
	...
	// BB developer: Max number of images per page
	$ngg_max_images_per_page = 100;
	...
	// Build navigation
	//$_GET['paged'] = isset($_GET['paged']) ? intval($_GET['paged']) : 0; //old 
	$GET['paged'] = intval($_GET['paged']); //BB Dev
	if ( $_GET['paged'] < 1 )
		$_GET['paged'] = 1;
	$start = ( $_GET['paged'] - 1 ) * $ngg_max_images_per_page; //BB Dev
	if ( $start < 1 )
		$start = 0;

	// Get the images
	if ( $galleryID != 0 )
		$picarray = $wpdb->get_col("SELECT pid FROM $wpdb->nggpictures WHERE galleryid = '$galleryID' AND exclude != 1 ORDER BY {$ngg->options['galSort']} {$ngg->options['galSortDir']} LIMIT $start, $ngg_max_images_per_page");	
...
<div class="tablenav">
	...
	$page_links = paginate_links( array(
		...
		'format' => '',
		'total' => ceil($total / $ngg_max_images_per_page),/*BB Dev*/
		'current' => $_GET['paged']
	));
	...
	<div class="alignleft actions">
		<select id="select_gal" name="select_gal" style="width:300px;" onchange="this.form.submit();"> <!--BB Dev: -->
		...
		<!-- BB Dev:<input type="submit" id="show-gallery" value="<?php esc_attr( _e('Select &#187;','nggallery') ); ?>" class="button-secondary" />-->
	</div>
	<br style="clear:both;" />
</div>
</form>
...
<!-- BB Dev -->		  <button style="float:right;margin-right:10px;margin-top:8px;" type="submit" class="button" value="1" name="send[<?php echo $picid ?>]"><?php echo esc_attr( __('Insert into Post') ); ?></button>
			  ...
			  <table class='slidetoggle describe startclosed'><tbody>
				  ...
				  <tr>
					...
					<tr class="image-size">
						...
						<td class="field">
							<input name="image[<?php echo $picid ?>][size]" id="image-size-thumb-<?php echo $picid ?>" type="radio" value="thumbnail" /><!--BB Dev: 'checked' removed !-->
							<label for="image-size-thumb-<?php echo $picid ?>"><?php esc_attr_e("Thumbnail") ; ?></label>
							<input name="image[<?php echo $picid ?>][size]" id="image-size-full-<?php echo $picid ?>" type="radio" checked="checked" value="full" /><!--BB Dev: 'checked' added !-->
